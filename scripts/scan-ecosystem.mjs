...
async function scanPackage(spec) {
  ...
  for (const e of entries) {
    ...
    if (rel === 'package.json') {
      try {
        packageJson = JSON.parse(e.content.toString('utf-8'));
      } catch {
        /* ignore */
      }
      continue;
    ...
    if (e.content.length > MAX_FILE_BYTES || files.length >= MAX_FILES) continue;
    files.push({ path: e.name, relPath: rel, content: e.content.toString('utf-8') });
  }
  ...
  return { spec, fileCount: files.length, version: packageJson?.version ?? version ?? '?', findings: summary.findings };
}
...
async function main() {
  ...
  for (const spec of packages) {
    ...
    try {
      ...
      results.push(r);
      console.log(`${r.fileCount} files, ${r.findings.length} findings ${tally(r.findings)}`);
    } catch (err) {
...
function report(results, errors) {
  ...
  const withAny = results.filter((r) => r.findings.length > 0).length;
  console.log(`\nPackages scanned successfully: ${scanned}`);
  console.log(`Packages with >=1 finding:    ${withAny} (${pct(withAny, scanned)})`);
  ...
  console.log('\nPer package:');
  for (const r of results) {
    const ids = [...new Set(r.findings.map((f) => f.ruleId))].sort().join(', ') || 'clean';
